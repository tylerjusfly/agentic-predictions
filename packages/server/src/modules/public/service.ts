  ...
  public getPublicPremierLeagueMatches = async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const PredictedData = (await dbAll(
        "SELECT id, game_id, home_team, away_team, confidence_level, time, date, actual_score FROM premierleague WHERE month = ? AND actual_score IS NULL ORDER BY time ASC LIMIT 5",
        [currentMonth]
      )) as ResponseResult[];
